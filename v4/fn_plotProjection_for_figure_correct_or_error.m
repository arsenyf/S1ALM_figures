function  POUT = fn_plotProjection_for_figure_correct_or_error (Param, rel, key_mode, trial_type_substract, ylab, mode_names_titles, flag_normalize_modes, plot_r_flag, line_style, stim_time_flag, key_trial_type)
hold on;
...
%     idx_pure_r = find(strcmp('r',POUT.trial_types));
    ...
    max_r=nanmax(Proj_right_hit(time>-4 & time<2));
    if max_r<0
...
for itype= 1:1:numel(trial_types)
    ...
    if line_style==':'
            plot(time,POUT.p(itype,:),'Color',POUT.rgb(itype,:) ,'LineWidth',1.5,'LineStyle',line_style);
    else
    plot(time,POUT.p(itype,:),'Color',POUT.rgb(itype,:) ,'LineWidth',0.75,'LineStyle',line_style);
    end
end
