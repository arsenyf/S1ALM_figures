...
% dir_save_figure = 'C:\install\Dropbox (HHMI)\S1ALM\Manuscript\figures\v4\';
filename = 'Fig1';
...
position_y2(4)=position_y2(3)-vertical_distance2*1.25;
...
imagesc(fig1_a);
set(gca,'Xlim',xl,'Ylim',yl);
text(xl(1)-diff(xl)*0.2, yl(1)-diff(yl)*0.02, 'a', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
axis off;
axis tight;
% axis equal;
...
%% Mouse brain
axes('position',[position_x1(3)-0.04 position_y1(2)-0.01  panel_width1*1.3 panel_height1*1.3]);
xl = [0 932];
yl = [0 982];
...
imagesc(fig1_a);
set(gca,'Xlim',xl,'Ylim',yl);
text(xl(1)+diff(xl)*0.01, yl(1)+diff(yl)*0.02, 'c', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
axis off;
axis tight;
axis equal;
...
%% Task schematics
axes('position',[position_x1(2)-0.01 position_y1(2)+0.03  panel_width1*0.8 panel_height1]);
xl = [0 278];
yl = [0 184];
...
axes('position',[position_x1(2)-0.05 position_y1(2)-0.005  panel_width1*1.5 panel_height1*0.35]);
xl=[-3.5 1];
yl=[0 1];
hold on;
time=-5:0.01:5;
...
xlim(xl);
ylim(yl);
set(gca, 'Xtick', [-3:1:0], 'Ytick',[], 'TickLabelInterpreter', 'None', 'FontSize', 7, 'XTickLabelRotation', 0,'TickDir','out');
...
text(xl(1)-diff(xl)*0.01, yl(1)+diff(yl)*3.4, 'b', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
box off
set(gca,'YColor','none');

%% Sound icons
axes('position',[position_x1(2)-0.05 position_y1(2)-0.005  panel_width1*0.15 panel_height1*0.2]);
...
axes('position',[position_x1(2)-0.02 position_y1(2)-0.005  panel_width1*0.15 panel_height1*0.2]);
fig1_a = imread([dir_embeded_graphics 'sound.jpg']);
imagesc(fig1_a);
axis tight;
axis equal;
...
axes('position',[position_x1(2)+0.06 position_y1(2)-0.005  panel_width1*0.15 panel_height1*0.2]);
fig1_a = imread([dir_embeded_graphics 'sound.jpg']);
imagesc(fig1_a);
axis tight;
axis equal;
...
imagesc(fig1_a);
set(gca,'Xlim',xl,'Ylim',yl);
text(xl(1)-diff(xl)/3.2, yl(1)+diff(yl)*1.15, 'd', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
...
k.unit_uid=2124; x=2; y=1; flag_xlable=0; flag_ylable=1; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 2) %2122, 2097
k.unit_uid=2143; x=3; y=1; flag_xlable=0; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 3)
k.unit_uid=4915; x=4; y=1; flag_xlable=0; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 4) %2260, 4879

%%Left ALM
k.unit_uid=1811; x=1; y=2; flag_xlable=0; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 5)
k.unit_uid=2018; x=2; y=2; flag_xlable=0; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 6) %1915, 2644, 4020, 2018
k.unit_uid=2591; x=3; y=2; flag_xlable=0; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 7) %2717 2015, 2723
k.unit_uid=1833; x=4; y=2; flag_xlable=0; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 8) %2402, 2733, 2695, 2655, 2331,2695, 4526, 4457, 2750, 2043, 1945, 1871, 1844

%%Right ALM
k.unit_uid=4822; x=1; y=3; flag_xlable=1; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 9)
k.unit_uid=4666; x=2; y=3; flag_xlable=1; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 10) %4757
k.unit_uid=4720; x=3; y=3; flag_xlable=1; flag_ylable=0; figure_plot_PSTH(k,x,y,Param, rel, flag_xlable, flag_ylable, 11) %4611
...
key.cell_type = 'Pyr';
axes('position',[position_x2(6), position_y2(1), panel_width2*1.1, panel_height2]);
labels_flag=0;
[xl,yl]=figure_plotSpikeCount (Param, rel, labels_flag, key);

text(xl(1)-diff(xl)*0.3, yl(1)+diff(yl)*1.25, ['{\color{green}Selectivity ' ...
    '\color{black}('...
    '\color{blue}Right'...
        '\color{black} - '...
...
text(-0.9, yl(1)+diff(yl)*1.0, 'Delay', ...
...
key.training = 'regular';
key.cell_type = 'Pyr';
axes('position',[position_x2(6), position_y2(2), panel_width2*1.1, panel_height2]);
labels_flag=0;
[xl,yl]=figure_plotSpikeCount (Param, rel, labels_flag, key);
text(xl(1)-diff(xl)*0.35, yl(1)+diff(yl)*1.05, 'g', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
...
figure_plot_svm(svm_performance,t,legend_flag,key,panel_label);
...
last_label=0;
c_lims=[50 75];
axes('position',[position_x2(4)+0.035, position_y2(4), panel_width2, panel_height2]);
svm_performance_mat = (fetchn(rel & key,'svm_performance_mean_time_mat'));
time_mat=[];
for i=1:1:size(svm_performance_mat,1)
    time_mat(i,:,:)=svm_performance_mat{i};
...
time_mat = 100*squeeze(mean(time_mat,1));
figure_plot_svm_time_rotation(t, time_mat, position_x2(4)+0.035, position_y2(4), panel_width2, panel_height2, label_flag, c_lims, last_label, key);
% freezeColors;
...
last_label=1;
c_lims=[50 90];
axes('position',[position_x2(5)+0.04, position_y2(4), panel_width2, panel_height2]);
svm_performance_mat = (fetchn(rel & key,'svm_performance_mean_time_mat'));
time_mat=[];
for i=1:1:size(svm_performance_mat,1)
    time_mat(i,:,:)=svm_performance_mat{i};
...
time_mat = 100*squeeze(mean(time_mat,1));
figure_plot_svm_time_rotation(t, time_mat, position_x2(5)+0.04, position_y2(4), panel_width2, panel_height2, label_flag, c_lims, last_label, key);
% freezeColors;
