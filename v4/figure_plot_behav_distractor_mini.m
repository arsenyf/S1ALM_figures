function [xl,yl]=figure_plot_behav_distractor_mini(behav_param, behav_param_mean, behav_param_signif,  x_r, x_l, y_r, y_l, trn_r, trn_l, flag_plot_left_right_trials, plot_signif_offset,names_right_trials, names_left_trials, legends)
hold on;
...
%     ylabel([sprintf('\\Delta probability\n lick right (%%)')],'FontSize',7);
...
if (flag_plot_left_right_trials ==0 || flag_plot_left_right_trials ==1)
    ...
    text(-2.5, 40 ,sprintf('Sample'),'FontSize',6,'HorizontalAlignment','left','Color',[0 0 1],'Rotation',90);
    early_delay_colr=fetch1(ANL.TrialTypeGraphic &  'trial_type_name="l_-1.6Full"','trialtype_rgb');
    text(-1.8, 45 ,sprintf('Early\nDelay'),'FontSize',6,'HorizontalAlignment','left','Color',early_delay_colr);
...
x = 0:0.01:pi;
plot(linspace(-2.5,-2.4,numel(x)),sin(x)*(factor/3)+70,'Color',[0.6 0.85 1],'LineWidth',1,'Clipping','off')
plot(linspace(-1.6,-1.5,numel(x)),sin(x)*(factor/3)+70,'Color',[0.6 0.85 1],'LineWidth',1,'Clipping','off')
plot(linspace(-0.8,-0.7,numel(x)),sin(x)*(factor/3)+70,'Color',[0.6 0.85 1],'LineWidth',1,'Clipping','off')
