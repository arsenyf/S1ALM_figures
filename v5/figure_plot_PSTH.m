...
if flag_ylable==1
    ...
    plot([xl(1)+diff(xl)*1.25 xl(1)+diff(xl)*1.25+1], [yl(1)+diff(yl)*2.5 yl(1)+diff(yl)*2.5],'-','LineWidth',1,'Color',[1 0 0],'Clipping','off');
    text(xl(1)+diff(xl)*2.9,yl(1)+diff(yl)*2.5,'Lick Right','FontSize',7,'Color',[0 0 1]);
    plot([xl(1)+diff(xl)*2.7 xl(1)+diff(xl)*2.7+1], [yl(1)+diff(yl)*2.5 yl(1)+diff(yl)*2.5],'-','LineWidth',1,'Color',[0 0 1],'Clipping','off');
