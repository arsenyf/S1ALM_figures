...
mode_names = {'LateDelay', 'Ramping Orthog.1'};
...
rel{1} =((EXP.Session * EXP.SessionID * ANL.ProjTrialAdaptiveAverage * EXP.SessionTraining  * ANL.TrialTypeID * ANL.TrialTypeGraphic * ANL.TrialTypeInstruction *  ANL.IncludeSession * ANL.SessionGrouping * ANL.TrialTypeStimTime - ANL.ExcludeSession) & k & 'unit_quality!="multi"' );
k.mode_weights_sign = 'all';
rel{2} =((EXP.Session * EXP.SessionID * ANL.ProjTrialAdaptiveAverage * EXP.SessionTraining  * ANL.TrialTypeID * ANL.TrialTypeGraphic * ANL.TrialTypeInstruction *  ANL.IncludeSession * ANL.SessionGrouping * ANL.TrialTypeStimTime - ANL.ExcludeSession) & k & 'unit_quality!="multi"' );
...
hold on;
switch_prob1 =  movmean(switch_prob ,[10 0], 2, 'omitnan','Endpoints','shrink');
...
normalized_proj_bins =  cell2mat(fetchn(rel_Switch,'normalized_proj_bins' ,'ORDER BY session_uid'));
hold on;
switch_prob2 =  movmean(switch_prob ,[10 0], 2, 'omitnan','Endpoints','shrink');
x=nanmedian(normalized_proj_bins);
...
xlim(xl);
ylim(yl);
set(gca,'FontSize',7,'Xtick',[0.25 0.75],'Ytick',yl);
...
set(gca,'Xlim',xl,'Ylim',yl,'Xtick',[1 2 3.5 4.5],'XtickLabel',{'Mini   ','Full','Mini  ',' Full'},'FontSize',7);
