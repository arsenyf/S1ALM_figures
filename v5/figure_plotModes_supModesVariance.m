function [xl, yl] = figure_plotModes_supModesVariance (rel,Param, xlabel_flag, title1, title2,title3, ylabel_flag, mode_names, mode_names_titles, y_lims, outcome, flag_normalize_modes, plot_r_flag, xl, line_style, stim_time_flag)
...
for imod = 1:1:numel(mode_names)
    ...
    ylim(yl);
    if ylabel_flag==1
        text ((xl(1)-diff(xl)*0.15), (yl(1) + diff(yl)*0.5), sprintf('%s mode\n   (a.u.)',mode_names_titles{imod}),'Fontsize', 8,'Rotation',90,'VerticalAlignment','middle','HorizontalAlignment','center');
    end
    
    set(gca,'FontSize',8,'YTick',[0,y_lims(2)]);
    
    %     set(gca,'FontSize',10,'YTick',[0,ax_lims(imod, 2)]);
    %     set(gca,'FontSize',10,'YTick',[-15, 5]);
    
end
text (-2, (yl(2) + diff(yl)*0.6), sprintf('%s\n%s\n%s',title3,title1, title2),'Fontsize', 8,'HorizontalAlignment','center','FontWeight','bold');
