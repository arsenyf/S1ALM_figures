function  figure_plot_svm(svm_performance,t,legend_flag,key, panel_label)
...
ylim(yl);
 if legend_flag==1
    ...
    text(-5,40,'Time (s)','FontSize',7,'HorizontalAlignment','left');
    text(xl(1)-diff(xl)*0.4, yl(1)+diff(yl)*0.5,sprintf('Decoder perform. \n(%%)'), 'FontSize',7,'HorizontalAlignment','center','Rotation',90);
    text(xl(1)-diff(xl)*0.8, yl(1)+diff(yl)*1.2, panel_label, ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
   else
    text(xl(1)-diff(xl)*0.2, yl(1)+diff(yl)*1.2, panel_label, ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
end
