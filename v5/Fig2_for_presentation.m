...
filename = 'Fig2_for_presentation';
...
position_y2(1)=0.78;
position_y2(2)=position_y2(1)-vertical_distance2*1.5;
position_y2(3)=position_y2(2)-vertical_distance2*2;
...
axes('position', [position_x2(1)-0.11 position_y2(2)+0.02 0.06 0.06]); hold on ;
xl = [0 121];
yl = [0 115];
...
text(xl(1)-diff(xl)*0.55, yl(1)+diff(yl)*1.75, '', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
if label_flag==1
...
text(xl(1)-diff(xl)*0.55, yl(1)+diff(yl)*1.3,'' , ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
...
text(xl(1)-diff(xl)*0.55, yl(1)+diff(yl)*1.15, '', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
...
compute_stim_amp (rel_input_mini_q,  Param, colr, flag_normalize_modes, plot_r_flag, yl, signif_display_offset, key);
xl=[-2.9 0.5];
xlim(xl);
set(gca,'Xtick',[-2 0], 'Ytick',[yl(1) yl(2)], 'YColor',colr,'Fontsize', 6)
...
compute_stim_amp (rel_input_full_q,  Param, colr, flag_normalize_modes, plot_r_flag, yl, signif_display_offset, key);
set(gca,'Xtick',[-2 0], 'Ytick',[yl(1) yl(2)], 'YColor',colr,'Fontsize', 6)
...
text(xl(1)-diff(xl)*0.55, yl(1)+diff(yl)*1.05, '', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
...
% Distractor full
colr = legends{3}.colr;
compute_stim_kinetics_modes4 (rel_choice_full,  Param, colr, flag_normalize_modes, key);

% Regular mini
colr = legends{1}.colr;
compute_stim_kinetics_modes4 (rel_choice_mini,  Param, colr, flag_normalize_modes, key);
...
text(xl(1)+diff(xl)*-0.7, yl(1)+diff(yl)*1.2, '', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
