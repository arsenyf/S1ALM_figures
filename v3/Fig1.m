...
set(gcf,'PaperUnits','centimeters','PaperPosition',[0.5 -5 23*1.2 25*1.4]);
...
imagesc(fig1_a);
set(gca,'Xlim',xl,'Ylim',yl);
text(xl(1)+diff(xl)*0.01, yl(1)+diff(yl)*0.02, 'b', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
axis off;
axis tight;
axis equal;
...
text(xl(1)-diff(xl)*0.01, yl(1)+diff(yl)*3.4, 'c', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
box off
set(gca,'YColor','none');
...
text(xl(1)-diff(xl)*0.5, yl(1)+diff(yl)*1.05, 'g', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
