...
mode_names = {'Ramping Orthog.', 'LateDelay'};
mode_names_titles = {'Ramping', 'Choice'};
...
xl_normalized = [-0.15,1.05]; yl_normalized=[-0.15,1.05];
[xl,yl]=figures_plotModes_2D (rel,  Param, mode_names, mode_names_titles, xlabel_flag, ylabel_flag, flag_normalize_modes, xl_normalized, yl_normalized,outcome);
text(xl(1)+diff(xl)*-0.33, yl(1)+diff(yl)*1.2, 'd', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
title(sprintf('%s \n mini stimuli', legends{1}.title_label),'FontSize',7,'Color',legends{1}.colr);
text(xl(1)+diff(xl)*0.4, yl(1)+diff(yl)*0.9, 'Correct', ...
...
text(xl(1)+diff(xl)*-0.33, yl(1)+diff(yl)*1.2, 'e', ...
    'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
text(xl(1)+diff(xl)*0.4, yl(1)+diff(yl)*0.9, 'Error', ...
    'fontsize', 7,'HorizontalAlignment','center');
