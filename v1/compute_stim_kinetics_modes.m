function [OUT, GP] = compute_stim_kinetics_modes (rel,Param, colr)
...
if strcmp(key_mode.outcome,'hit')
    min_num_trials_projected = 1.5*min_num_trials_projected_correct;
else
...
%     end
    ...
    add_NaN_aftert_go_cue(:,time(t_idx)>=2)=NaN;
    idx = size(STIM,1);
%     STIM (:,:,counter)  = (S(:,t_idx)- Proj_Baseline(:,t_idx)) +add_NaN_aftert_go_cue +repmat(add_NaN,1,sum(t_idx));
